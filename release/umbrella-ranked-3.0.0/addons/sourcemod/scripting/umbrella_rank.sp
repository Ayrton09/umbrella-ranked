...
#define RANK_MIN_KILLS 1
#define RANK_TOP1_SOUND "buttons/bell1.wav"
#define RANK_COMMAND_COOLDOWN 3.0
#define RANK_AUTOSAVE_INTERVAL 120.0
#define RANK_RESET_COOLDOWN_DAYS 30
#define RANK_START_POINTS 1000
#define RANK_MIN_POINTS 0
#define RANK_KILL_BASE_POINTS 2
#define RANK_KILL_MIN_POINTS 1
#define RANK_KILL_MAX_POINTS 15
#define RANK_POINTS_DIFF_STEP 100
#define RANK_DEATH_POINTS_MULTIPLIER 1.0
#define RANK_HEADSHOT_BONUS 1
#define RANK_DOMINATION_BONUS 2
#define RANK_REVENGE_BONUS 1
#define RANK_KNIFE_MULTIPLIER 2.0
#define RANK_TEAMKILL_PENALTY 3
#define RANK_SUICIDE_PENALTY 2
#define RANK_BOMB_PLANTED_POINTS 3
#define RANK_BOMB_DEFUSED_POINTS 5
#define RANK_BOMB_EXPLODED_POINTS 4
#define RANK_HOSTAGE_RESCUED_POINTS 2
...
void ResetClientData(int client)
{
    g_iKills[client] = 0;
    g_iDeaths[client] = 0;
    g_iPoints[client] = RANK_START_POINTS;
...
{
    int diffBonus = (g_iPoints[victim] - g_iPoints[attacker]) / RANK_POINTS_DIFF_STEP;
    int points = RANK_KILL_BASE_POINTS + diffBonus;

    points = ClampInt(points, RANK_KILL_MIN_POINTS, RANK_KILL_MAX_POINTS);

    if (headshot)
    {
        points += RANK_HEADSHOT_BONUS;
    }

    if (dominated)
    {
        points += RANK_DOMINATION_BONUS;
    }

    if (revenge)
    {
        points += RANK_REVENGE_BONUS;
    }

    if (IsKnifeKill(weapon))
    {
        points = RoundToCeil(float(points) * RANK_KNIFE_MULTIPLIER);
    }
...
bool CheckCooldown(int client)
{
    ...
    float cooldown = RANK_COMMAND_COOLDOWN;
...
bool CanUseResetRank(int client, bool showMessages = true, bool checkCooldown = true)
{
    ...
    int cooldownDays = RANK_RESET_COOLDOWN_DAYS;
    int now = GetTime();
...
void StartResetRankNow(int client)
{
    ...
    int resetPoints = (g_iPoints[client] < RANK_START_POINTS) ? g_iPoints[client] : RANK_START_POINTS;
...
void FlushWeaponStats(int client)
{
    ...
    int weaponCount = 0;
    char query[4096], part[160], escWeapon[64];
...
void StartAutoSaveTimer()
{
    ...
    float interval = RANK_AUTOSAVE_INTERVAL;
    if (interval <= 0.0)
    {
        return;
    }
...
public void SQL_OnDataLoaded(Database db, DBResultSet results, const char[] error, any data)
{
    ...
    if (IsRankActive() && g_iKills[client] >= RANK_MIN_KILLS)
    {
...
public Action Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
    ...
    bool dominated = (event.GetInt("dominated") != 0);
    bool revenge = (event.GetInt("revenge") != 0);
    ...
    if (validAttacker && atk != vic)
    {
        ...
        if (teamKill)
        {
            int penalty = RANK_TEAMKILL_PENALTY;
        ...
        int victimLoss = RoundToCeil(float(points) * RANK_DEATH_POINTS_MULTIPLIER);
    ...
    int suicidePenalty = RANK_SUICIDE_PENALTY;
    g_iDeaths[vic]++;
    g_iSessionDeaths[vic]++;
    g_iSuicides[vic]++;
...
public Action Event_BombPlanted(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (IsRankActive() && IsValidRankClient(client))
    {
        ...
        int appliedGain = AwardObjectivePoints(client, RANK_BOMB_PLANTED_POINTS);
        if (appliedGain > 0)
        {
...
public Action Event_BombDefused(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (IsRankActive() && IsValidRankClient(client))
    {
        ...
        int appliedGain = AwardObjectivePoints(client, RANK_BOMB_DEFUSED_POINTS);
        if (appliedGain > 0)
        {
...
public Action Event_BombExploded(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (IsRankActive() && IsValidRankClient(client))
    {
        ...
        int appliedGain = AwardObjectivePoints(client, RANK_BOMB_EXPLODED_POINTS);
        if (appliedGain > 0)
        {
...
public Action Event_HostageRescued(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (IsRankActive() && IsValidRankClient(client))
    {
        ...
        int appliedGain = AwardObjectivePoints(client, RANK_HOSTAGE_RESCUED_POINTS);
        if (appliedGain > 0)
        {
...
public Action Command_Rank(int client, int args)
{
    ...
    Format(query, sizeof(query),
        ...
        RANK_MIN_KILLS, RANK_MIN_KILLS, escAuth
    );

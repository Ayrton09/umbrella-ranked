...
- Automatic database reconnect and load/save retries.
- Translated to English, Spanish, Portuguese, Russian, and Chinese.
- Compatible with CS:GO and CS:S (CS:GO-only events such as assist/MVP/taser are skipped on CS:S).

## Requirements

- CS:GO or Counter-Strike: Source dedicated server.
- SourceMod 1.11 or newer (built and tested with 1.12).
- A MySQL server or SQLite (bundled with SourceMod, no extra setup).
...
The repository includes the required MultiColors include files, so the plugin compiles with a stock SourceMod compiler:

```text
spcomp addons/sourcemod/scripting/umbrella_rank.sp -i addons/sourcemod/scripting/include
```
...
- Player data loads when the client is authenticated; failed loads are retried automatically (up to 3 times).
- If the database is unreachable at startup, the plugin retries the connection every 30 seconds and loads pending players once it succeeds.
- Stats are saved on a configurable autosave interval (`sm_rank_autosave_interval`, default 120 s), on disconnect, and on map end.
- Player saves are idempotent and ordered: a stale or retried save can never overwrite newer data (guarded by `last_seen`/`save_seq`).
- Weapon kills are batched in memory and flushed in a single query; a batch that fails is dropped and logged rather than retried, so kills can never be double-counted.
- While a rank reset is being processed, saves and weapon flushes for that player are paused until the reset completes.
...
| `sm_rank_bare_triggers` | `1` | `1` = chat words like `rank` or `top` open the stats without an `!`/`/` prefix. `0` = require `!`/`/`. |
...
By default these triggers also work as plain chat words (e.g. typing `top` or `rank`), controlled by `sm_rank_bare_triggers`.
...
All chat messages and menus are translated via `umbrella_ranked.phrases.txt`:
English (`en`), Spanish (`es`), Portuguese (`pt`), Russian (`ru`), and Chinese (`chi`).
Each player sees the messages in their own client language.

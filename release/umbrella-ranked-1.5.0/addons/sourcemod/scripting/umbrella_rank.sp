...
#define RANK_DB_CONNECT_RETRY_DELAY 30.0
...
int g_iLoadRetryCount[MAXPLAYERS + 1];
...
int g_iWeaponFlushesInFlight[MAXPLAYERS + 1];
...
// El cooldown va primero: es el único freno anti-spam de las respuestas de error.
bool CanUseRankCommand(int client, bool requireActive)
{
    if (client <= 0 || !IsClientInGame(client))
    {
        ReplyToCommand(client, "[Umbrella Ranked] This command can only be used in-game.");
        return false;
    }
    ...
    if (requireActive && !CheckRankAvailability(client))
    {
        return false;
    }

    if (g_hDatabase == null || !g_bDataLoaded[client] || g_bResetInProgress[client])
    {
...
void UpdateSessionPlayTime(int client)
{
    ...
    // Segmentos >= 24h se descartan: son saltos de reloj (NTP/resume), no juego real.
...
bool CanUseResetRank(int client, bool showMessages = true, bool checkCooldown = true)
{
    ...
    if (g_iWeaponFlushesInFlight[client] > 0)
    {
        if (showMessages)
        {
            CPrintToChat(client, "%t", "Data Loading");
...
public void SQL_OnConnect(Database db, const char[] error, any data)
{
    if (db == null)
    {
        LogError("[Umbrella Ranked] No se pudo conectar a '%s': %s. Reintentando en %.0f segundos.", RANK_DB_CONNECTION, error, RANK_DB_CONNECT_RETRY_DELAY);
        CreateTimer(RANK_DB_CONNECT_RETRY_DELAY, Timer_RetryConnect);
        return;
    }
...
void RunDatabaseMigrationStep(int step)
{
    ...
    switch (step)
    {
        ...
        case 15:
        {
            Format(query, sizeof(query), "CREATE INDEX %sidx_player_rank ON player_stats (points, kills, deaths, playtime, name)", StrEqual(g_szDriver, "sqlite") ? "IF NOT EXISTS " : "");
        }
        case 16:
        {
            Format(query, sizeof(query), "CREATE INDEX %sidx_player_playtime ON player_stats (playtime, name)", StrEqual(g_szDriver, "sqlite") ? "IF NOT EXISTS " : "");
        }
        case 17:
        {
            Format(query, sizeof(query), "CREATE INDEX %sidx_weapon_rank ON weapon_stats (weapon, kills)", StrEqual(g_szDriver, "sqlite") ? "IF NOT EXISTS " : "");
        }
        default:
        {
            FinishDatabaseSetup();
...
void FinishDatabaseSetup()
{
    for (int i = 1; i <= MaxClients; i++)
    {
        ...
        {
            // Solo inicializar si el slot nunca pasó por OnClientPostAdminCheck
            // (carga tardía del plugin); si ya pasó, conserva su hora de conexión.
...
DataPack CreateClientSavePack(const char[] auth, const char[] query, int retryCount)
{
    DataPack pack = new DataPack();
...
void ScheduleClientSaveRetry(const char[] auth, const char[] query, int retryCount)
{
    DataPack retry = CreateClientSavePack(auth, query, retryCount);
...
public Action Timer_RetryClientSave(Handle timer, any data)
{
    ...
    if (g_hDatabase == null)
    {
        ...
        {
            ScheduleClientSaveRetry(auth, query, retryCount + 1);
        }
        else
        {
...
public void SQL_OnClientDataSaved(Database db, DBResultSet results, const char[] error, any data)
{
    ...
    if (error[0] != '\0')
    {
        ...
        // Reintentar es seguro: el upsert lleva guard de last_seen/save_seq (idempotente).
        if (retryCount < RANK_DB_MAX_RETRIES)
        {
            ScheduleClientSaveRetry(auth, query, retryCount + 1);
        }
...
void SaveClientData(int client, bool allowDisconnected = false)
{
    if (g_hDatabase == null || client <= 0 || client > MaxClients || IsFakeClient(client) || !g_bDataLoaded[client] || g_bResetInProgress[client])
    {
        return;
    }
    ...
    DataPack pack = CreateClientSavePack(auth, query, 0);
    g_hDatabase.Query(SQL_OnClientDataSaved, query, pack);
}
...
{
    if (g_hDatabase == null || client <= 0 || client > MaxClients || IsFakeClient(client) || g_iWeaponDeltaCount[client] <= 0 || g_bResetInProgress[client])
    {
        return;
    }
    ...
    for (int i = 0; i < g_iWeaponDeltaCount[client]; i++)
    {
        ...
        if (!EscapeSqlString(g_hDatabase, g_szWeaponDeltaName[client][i], escWeapon, sizeof(escWeapon), "weapon name"))
        {
            return;
        }
        Format(part, sizeof(part), "%s('%s', '%s', %d)", (weaponCount == 0) ? "" : ", ", escAuth, escWeapon, g_iWeaponDeltaKills[client][i]);
    ...
    // Los deltas pasan a ser propiedad de la query en vuelo: no se reintentan
    // ante error de query (el upsert aditivo no es idempotente y reenviar
    // podría duplicar kills si la query sí se aplicó).
    g_iWeaponFlushesInFlight[client]++;
    ClearWeaponDeltas(client);
...
public void SQL_OnWeaponStatsFlushed(Database db, DBResultSet results, const char[] error, any data)
{
    ...
    if (error[0] != '\0')
    {
        LogError("[Umbrella Ranked] Error saving weapon stats for %s (batch dropped, not retried): %s", auth, error);
    }

    // El userid identifica la conexión de forma única: si sigue resolviendo,
    // el contador pertenece a esta misma sesión. No condicionar al auth, que
    // puede fallar transitoriamente y dejaría el contador clavado.
    int client = GetClientOfUserId(userid);
    if (client != 0 && g_iWeaponFlushesInFlight[client] > 0)
    {
        g_iWeaponFlushesInFlight[client]--;
    }
}
...
public void SQL_OnDataLoaded(Database db, DBResultSet results, const char[] error, any data)
{
    ...
    if (error[0] != '\0')
    {
        ...
        if (g_iLoadRetryCount[client] < RANK_DB_MAX_RETRIES)
        {
            g_iLoadRetryCount[client]++;
            CreateTimer(RANK_DB_RETRY_DELAY, Timer_RetryLoadClientData, data);
        }
        else
        {
            LogError("[Umbrella Ranked] Load retry exhausted for %N.", client);
        }
    ...
    bool firstLoad = !g_bDataLoaded[client];
    g_bDataLoaded[client] = true;
    g_iLoadRetryCount[client] = 0;
    g_iSessionStart[client] = GetTime();

    if (firstLoad && IsRankActive() && g_iKills[client] >= g_cvMinKills.IntValue)
    {
        CreateTimer(3.0, Timer_CheckWelcome, GetClientUserId(client));
    }
}

public Action Timer_RetryLoadClientData(Handle timer, any userid)
{
    int client = GetClientOfUserId(userid);
...
{
    if (!CanUseRankCommand(client, true))
    {
        return Plugin_Handled;
    }
...
{
    if (!CanUseRankCommand(client, false))
    {
        return Plugin_Handled;
    }
